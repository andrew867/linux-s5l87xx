...
static int cs42_retailos_play_start(struct cs42l81 *c)
{
	...
	/*
	 * Reported, never enforced. Detection is a sensor: it can read wrong
	 * today and fail outright tomorrow, and refusing to play because of it
	 * makes a bad cable indistinguishable from a broken device. RetailOS
	 * can afford to gate here because its detect works; ours does not yet,
	 * and silence with no override is the worse failure.
	 */
	if (!cs42_headset_ready())
		dev_warn(&c->spi->dev,
			 "headset not detected — playing anyway (RetailOS would 42D364(0))\n");
...
static int cs42_codec_prepare(struct cs42l81 *c, unsigned int rate)
{
	...
	/*
	 * Same as the play latch: this used to return -ENODEV, which failed
	 * hw_params and left userspace with no PCM to open and no mixer to
	 * unmute - a dead end with nothing pointing at the jack detect that
	 * caused it.
	 */
	if (!cs42_headset_ready())
		dev_warn(&c->spi->dev,
			 "headset not detected — configuring anyway (RetailOS gates 570620)\n");
